...
| `io` | Files, paths, streams, basic OS IO | Raw file read/write, File wrapper open/write/read/write-line/read-line/flush/seek/tell/close, BigFile chunk reads, StreamWriter flush/close behavior, portable `file_size_bytes`, file exists/delete/rename, directory exists/make/remove, failed-open/null guards, EOF behavior, and portable path helpers are native-smoke gated. VM/runtime gates cover sorted directory listing, recursive listing, and glob filters; direct-native listing/glob/process-output fallbacks now return `null` until OS-backed parity lands. |
