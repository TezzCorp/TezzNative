...
| Stable stdlib candidates | Stable/Beta (smoke gated) | Core imports plus native math, string, raw/wrapped/line/stream file IO, directory lifecycle, direct `dir_list`, public `dir_list_rec`/`glob_list`, portable path, vector, arena, process run/output capture, and `time` clock/sleep/UTC-date smoke. Raw recursive/glob builtins still fail closed until backend parity lands |
