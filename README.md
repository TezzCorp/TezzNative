...
| Native executable flow | Beta (gated x64) | Windows/Linux SDKs build and run hello, loops/math module, strings/transforms, structs, raw, wrapped, line-oriented, and chunked stream file IO, portable paths, vectors, arenas, and time clock/sleep/UTC-date helpers |
...
| Stable stdlib candidates | Stable/Beta (smoke gated) | Core imports plus native math, string, raw/wrapped/line/stream file IO, portable path, vector, arena, and `time` clock/sleep/UTC-date smoke; local timezone formatting remains runtime-backend hardening |
...
raw, wrapped, line-oriented, and chunked stream file IO, portable path helpers, vectors, arenas, and time clock/sleep/UTC-date helpers:
