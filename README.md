...
| GPU/NPU/LLM/kernel modules | Experimental | `llm_core` now has gated CPU f64/f32 API lanes, f16/bf16 storage-lane matmul, int8/uint8/q4 quantized matmul, tensor/KV descriptors, tokenizer byte fixtures, fail-closed GPU/NPU gates, and an LLM benchmark source; TezzMind has Windows-native forward/training/checkpoint evaluation fixtures, while Linux native TezzMind lowering remains a tracked backend blocker. Production LLM training/serving and reasoning-AI claims still need real model formats, tokenizer compatibility, backend acceleration, held-out evaluation, and published benchmark results |
